// Main game logic for mobile

const canvas = document.getElementById('gameCanvas');
const ctx = canvas.getContext('2d');

// Responsive canvas sizing for mobile
const CELL_SIZE = Math.min(Math.floor((window.innerWidth - 60) / 20), 30);
const MAZE_COLS = 20;
const MAZE_ROWS = 15;
...
class Player {
    ...
    draw() {
        ...
        const radius = Math.max(CELL_SIZE / 3, 5);
        ...
        // Hat
        ctx.fillStyle = this.color;
        const hatHeight = Math.max(Math.floor(CELL_SIZE * 0.27), 5);
        ctx.fillRect(x + 5, y + 5, CELL_SIZE - 10, hatHeight);

        // Eyes
        const eyeRadius = Math.max(Math.floor(CELL_SIZE / 10), 2);
        ctx.fillStyle = COLORS.white;
        ctx.beginPath();
        ctx.arc(x + CELL_SIZE * 0.4, y + CELL_SIZE * 0.5, eyeRadius, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(x + CELL_SIZE * 0.6, y + CELL_SIZE * 0.5, eyeRadius, 0, Math.PI * 2);
        ctx.fill();

        const pupilRadius = Math.max(eyeRadius - 1, 1);
        ctx.fillStyle = COLORS.black;
        ctx.beginPath();
        ctx.arc(x + CELL_SIZE * 0.4, y + CELL_SIZE * 0.5, pupilRadius, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(x + CELL_SIZE * 0.6, y + CELL_SIZE * 0.5, pupilRadius, 0, Math.PI * 2);
        ctx.fill();

        // Mustache (only for Mario)
        if (this.color === COLORS.mario && CELL_SIZE >= 20) {
            ctx.strokeStyle = COLORS.black;
            ctx.lineWidth = Math.max(Math.floor(CELL_SIZE / 15), 1);
            ctx.beginPath();
            ctx.moveTo(x + CELL_SIZE * 0.33, y + CELL_SIZE * 0.67);
            ctx.lineTo(x + CELL_SIZE * 0.67, y + CELL_SIZE * 0.67);
            ctx.stroke();
        }
    }
}
...
class Game {
    ...
    setupControls() {
        ...
        document.getElementById('upBtn').addEventListener('click', (e) => {
            e.preventDefault();
            if (!this.gameOver) this.player.move(0, -1);
        });
        ...
        document.getElementById('leftBtn').addEventListener('click', (e) => {
            e.preventDefault();
            if (!this.gameOver) this.player.move(-1, 0);
        });
        ...
        // Keyboard controls (for testing in browser)
    ...
    drawMaze() {
        ...
        // Draw "GOAL" text
        ctx.fillStyle = COLORS.white;
        ctx.font = `bold ${Math.max(Math.floor(CELL_SIZE * 0.4), 10)}px Arial`;
